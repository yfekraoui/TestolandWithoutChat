...
$pa = $account2['rank_parrain'];
$mazo = $account2['mazo'];
$clan = $account2['clan'];
$ab = $account2['avatar_bloby'];
...
{
	if($type == "r") {
		$req = mysql_fetch_assoc(mysql_query('SELECT avatar_design FROM phpbb_users WHERE user_id = "'.$userdata['user_id'].'"'));
		$dad = explode(';', $req['avatar_design']);
		$rdad = $dad[1].';'.$dad[2].';'.$dad[3].';'.$dad[4].';'.$dad[5].';'.$dad[6];
		mysql_query('UPDATE phpbb_users SET avatar_design = "r;'.$rdad.'" WHERE user_id = "'.$userdata['user_id'].'"');
		echo 'Ok ton avatar est bien mis Deco/Reco du tchat ou clique sur mon profil pour le faire apparaitre sur le tchat !<br />
						<a href="/account/">Retour.</a><br />';
	}
	else if($type == "sp") {
		$req = mysql_fetch_assoc(mysql_query('SELECT avatar_design FROM phpbb_users WHERE user_id = "'.$userdata['user_id'].'"'));
		$dad = explode(';', $req['avatar_design']);
		...
		switch($userdata['ph']) {
			case 1:
			mysql_query('UPDATE phpbb_users SET avatar_design = "g;'.$rdad.'" WHERE user_id = "'.$userdata['user_id'].'"');
			echo 'Ok ton avatar est bien mis Deco/Reco du tchat ou clique sur mon profil pour le faire apparaitre sur le tchat !<br />
						<a href="/account/">Retour.</a><br />';
			break;
			case 2:
			mysql_query('UPDATE phpbb_users SET avatar_design = "zomb;'.$rdad.'" WHERE user_id = "'.$userdata['user_id'].'"');
			echo 'Ok ton avatar est bien mis Deco/Reco du tchat ou clique sur mon profil pour le faire apparaitre sur le tchat !<br />
						<a href="/account/">Retour.</a><br />';
			break;
			case 3:
			mysql_query('UPDATE phpbb_users SET avatar_design = "vamp;'.$rdad.'" WHERE user_id = "'.$userdata['user_id'].'"');
			echo 'Ok ton avatar est bien mis Deco/Reco du tchat ou clique sur mon profil pour le faire apparaitre sur le tchat !<br />
						<a href="/account/">Retour.</a><br />';
			break;
			default:
			echo 'SAPERLIPOPETTE LA COKETTE DES SOKETTE DU SKATE EN BROUETTE DE FOURCHETTE !! ';
		}
	}
	else {
		echo "UI C E PI KOI ENKOR HEINNNNNNNNN BZOU BZOU ROLOLA";
	}
}
else
{

...
else if($userdata['user_level'] == "1")
{
	...
	if($message != NULL)// si il y a quelque chose d'envoyer
	{	   
		$adresse='78.226.226.160';
		$port=5000;
		$socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
		socket_connect($socket,$adresse,$port);
		$paquet = "<msg c='".$message." ( ".$userdata['username']." depuis le site )' />";
		socket_write($socket,$paquet,strlen($paquet));
		socket_close($socket);
		echo 'Le message "<b>'.$_POST['message'].'</b>" a bien été envoyé mon bobby !';

			
	}
	else
	{
	echo 'Hey bob t\'es con ou bien tu fais exprès ? :hap:';
	}

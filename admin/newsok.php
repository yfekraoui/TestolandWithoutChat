...
{	
	...
	{	// petite securité en plus + ajout slahes pour eviter les bugs
		$titre = mysql_real_escape_string(htmlspecialchars(stripslashes($_POST['titre'])));
		$contenu = mysql_real_escape_string(htmlspecialchars(stripslashes($_POST['contenu'])));
		$auteur = mysql_real_escape_string(htmlspecialchars($_POST['auteur']));
		
		mysql_query("UPDATE news_accueil SET titre='" . $titre . "', text='" . $contenu . "', auteur='" . $auteur . "', date=NOW() WHERE id='1'");
		echo 'News Modifier ! <br /> Retour a l\'admin > <a href="\admin\"> Go !</a>';
	}
	else
	{

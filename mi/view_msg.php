...
<table width="560" border="0" cellpadding="0" cellspacing="0">
 ...
 <tr>
	...
	<img src="/img/spacer.gif" height="15" width="50" /><b><center>Voir un message</b></center>
  ...
  <tr>
    ...
    <td bgcolor="#FFF4D5"><table border="0" cellpadding="3" cellspacing="1" width="100%">
      ...
      <tr>
		...
		if($vms == 1)
		{
			$dms = mysql_query('SELECT * FROM mi WHERE mid_r = "'.$userdata['user_id'].'" AND id = "'.$msid.'"');
			$dms = mysql_fetch_assoc($dms);
			$de = mysql_query('SELECT username, mood, avatar_design FROM phpbb_users WHERE user_id = "'.$dms['mid_e'].'"');
			$de = mysql_fetch_assoc($de);
			if($dms['vue'] == 0){ mysql_query('UPDATE mi SET vue = "1" WHERE mid_r = "'.$userdata['user_id'].'" AND id = "'.$msid.'"');} 
			?>
			<table>
			...
			<center><?php echo '<a href="/annuaire/?mid='.$dms['mid_e'].'">'.$de['username'].'</a>'; ?></center>
			...
			<input type="hidden" name="rep_s" value="<?php echo stripslashes($reponse);?>" />
			<input type="hidden" name="rep_eid" value="<?php echo $dms['mid_e'];?>" />
			<center><input type="image" src="repondre.png" alt="reponse" border="0" /></center>
			</form>

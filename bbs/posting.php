...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $post_id )
	{
		$redirect = "topic.$phpEx?" . POST_POST_URL . "=$post_id";
		$post_append = "#$post_id";
	}
	else if ( $topic_id )
	{
		$redirect = "topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id";
		$post_append = '';
	}
	else if ( $forum_id )
	{
		$redirect = "bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id";
		$post_append = '';
	}
	else
	{
...
if ( ($result = $db->sql_query($sql)) && ($post_info = $db->sql_fetchrow($result)) )
{
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">')
		);
		$message .=  '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
	else
	{
...
if( $refresh || isset($HTTP_POST_VARS['del_poll_option']) || $error_msg != '' )
{
	...
	if( $preview )
	{
		...
		{
			$preview_message = $preview_message . '<br/><br/><br/><br/><br/><i>"'.utf8_decode($user_sig).'"</i>';
		}
...
$template->assign_vars(array(
	...
	'U_VIEW_FORUM' => append_sid("bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id"))
);
...
<table width="100%" cellspacing="0" border="0" cellpadding="0">
  <tr>
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;postorder=desc") : '', 

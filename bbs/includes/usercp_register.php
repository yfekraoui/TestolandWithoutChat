	...
	$mode == 'register' )
{
	...
	$strip_var_list = array('email' => 'email', 'icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website','avatar_design'=>'avatar_design', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code');
	...
	if ( $mode != 'editprofile' )  $sid_me['packs'] = '1';
	else {
		$sid_moi = @mysql_query('SELECT s.session_user_id, user_id,u.username,u.attributes,u.user_level,packs, u.avatar_design, u.mood FROM phpbb_sessions s,phpbb_users u WHERE u.user_id = "'.$user_id.'" AND s.session_user_id = u.user_id'); 
		$sid_me = @mysql_fetch_assoc($sid_moi);
	}
	...
	$npack = explode(',',$sid_me['packs']);
	$count_packs = count($npack);
	$i = 0;
	while($i < $count_packs) {
		$pack_req = mysql_query('SELECT * FROM phpbb_packs WHERE id='.$npack[$i]);
		$pack_fetch = mysql_fetch_assoc($pack_req);
	
		$packs['head'] = array_merge($packs['head'], explode(',',$pack_fetch['head']));
		$packs['body'] = array_merge($packs['body'], explode(',',$pack_fetch['body']));
		$packs['foot'] = array_merge($packs['foot'], explode(',',$pack_fetch['foot']));
	...
	$buf_ad = explode(';',$avatar_design);
	if($buf_ad[0] != 'r') $ok_ad = false;
	else {
		$total_head = count($packs['head']);
		$i = 0;
		while($packs['head'][$i] != $buf_ad[2] && $i <= $total_head) $i++;
		if($total_head < $i) $ok_ad = false;

		$total_body = count($packs['body']);
		$i = 0;
		while($packs['body'][$i] != $buf_ad[3] && $i <= $total_body) $i++;
		if($total_body < $i) $ok_ad = false;

		$total_foot = count($packs['foot']);
		$i = 0;
		while($packs['foot'][$i] != $buf_ad[4] && $i <= $total_foot) $i++;
		if($total_foot < $i) $ok_ad = false;

	}

	if(!$ok_ad) $avatar_design = 'r;1;1;1;1;1;1';
	...
	if ( (isset($HTTP_POST_VARS['avatargallery']) || isset($HTTP_POST_VARS['submitavatar']) || isset($HTTP_POST_VARS['cancelavatar'])) && (!isset($HTTP_POST_VARS['submit'])) )
	{
		...
		$avatar_design = stripslashes($avatar_design);
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $board_config['allow_namechange'] || $mode == 'register' )
	{
		...
		else if ( $username != $userdata['username'] || $mode == 'register')
		{
			...
			{
				$error = TRUE;
				$error_msg .= ( ( isset($error_msg) ) ? '<br />' : '' ) . $lang['pseudocarct'];
			}
			if (strlen($username) < 4 || strlen($username) > 15)
			{
				$error = TRUE;
				$error_msg .= ( ( isset($error_msg) ) ? '<br />' : '' ) . $lang['pseudlong'];
			}
			if (!$error)
			{
				$signeee = random1(1);
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			...
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', avatar_design='" . str_replace("\'", "''", $avatar_design) . "',  user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . "
				WHERE user_id = $user_id";
			if ( !($result = $db->sql_query($sql)) )
			{
		...
		else
		{
			...
			// Get current date
			//
			$user_ad  = $_POST['avatar_design'];

			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_icq, user_website, user_occ, user_from, user_interests, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, avatar_design, user_sexe, user_active, ban, online, sign)
				VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, '$avatar_design', 'inconnu', 1, '0', '0', '" . $sign_astro . "')";
...
else if ( $mode == 'editprofile' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
	...
	$avatar_design = $userdata['avatar_design'];
...
else
{
	...
	if (!empty($board_config['enable_confirm']) && $mode == 'register')
	{
		...
		$confirm_image = '<img src="' . append_sid("?mode=confirm&amp;id=$confirm_id") . '" alt="" title="" />';
	...
	if($avatar_design == '') $avatar_design = 'r;1;1;1;1;1;1';
	$template->assign_vars(array(
...
$template->assign_vars(array('TPL_SLOT_BLUE_TOP'=>'<table width="100%" cellspacing="0" border="0" cellpadding="0"> 
  <tr> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_header_left.gif" width="8" height="8"/></td> 
	<td background="http://img.chapatiz.com/index_body_header_tile.gif"><img src="http://img.chapatiz.com/spacer.gif" width="8" height="8"/></td> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_header_right.gif" width="8" height="8"/></td> 
  </tr> 
</table> 
<table width="100%" cellspacing="0" border="0" cellpadding="0"> 
 
  <tr> 
	<td width="3" background="http://img.chapatiz.com/index_body_left.gif"><img src="http://img.chapatiz.com/spacer.gif" width="3" height="3"/></td> 
	<td bgcolor="#FFFFFF"> 
	<!-- content -->',
'TPL_SLOT_WHITE_BOTTOM'=>'	<!-- end content --> 
	</td> 
	<td width="3" background="http://img.chapatiz.com/index_body_right.gif"><img src="http://img.chapatiz.com/spacer.gif" width="3" height="3"/></td> 
  </tr> 
</table> 
<table width="100%" cellspacing="0" border="0" cellpadding="0"> 
  <tr> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_footer_left.gif" width="8" height="8"/></td> 
	<td background="http://img.chapatiz.com/index_body_footer_tile.gif"><img src="http://img.chapatiz.com/spacer.gif" width="8" height="8"/></td> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_footer_right.gif" width="8" height="8"/></td> 
  </tr> 
</table>',
'TPL_SLOT_WHITE_TOP'=>'<table width="100%" cellspacing="0" border="0" cellpadding="0"> 
 <tr> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_footer_left_top.gif" width="8" height="8"/></td> 
	<td background="http://img.chapatiz.com/index_body_footer_tile_top.gif"><img src="http://img.chapatiz.com/spacer.gif" width="8" height="8"/></td> 
	<td width="8"><img src="http://img.chapatiz.com/index_body_footer_right_top.gif" width="8" height="8"/></td> 
  </tr> 
</table> 
<table width="100%" cellspacing="0" border="0" cellpadding="0"> 
  <tr> 

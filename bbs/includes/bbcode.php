...
function prepare_bbcode_template($bbcode_tpl)
{
	...
	$bbcode_tpl['hs_username_open'] = str_replace('{L_HS}', $lang['Hs'], $bbcode_tpl['hs_username_open']);
	$bbcode_tpl['hs_username_open'] = str_replace('{L_WROTE}', $lang['wrote'], $bbcode_tpl['hs_username_open']);
	$bbcode_tpl['hs_username_open'] = str_replace('{USERNAME}', '\\1', $bbcode_tpl['hs_username_open']);
...
function bbencode_second_pass($text, $uid)
{
	...
	// [HS] and [/HS] for posting replies with hs, or just for quoting stuff.
	$text = str_replace("[hs:$uid]", $bbcode_tpl['hs_open'], $text);
	$text = str_replace("[/hs:$uid]", $bbcode_tpl['hs_close'], $text);

	// New one liner to deal with opening HS with usernames...
	// replaces the two line version that I had here before..
	$text = preg_replace("/\[Hors Sujet:$uid=\"(.*?)\"\]/si", $bbcode_tpl['hs_username_open'], $text);
...
function bbencode_first_pass($text, $uid)
{
	...
	$text = bbencode_first_pass_pda($text, $uid, '/\[quote=\\\\&quot;(.*?)\\\\&quot;\]/is', '[/quote]', '', false, '', "[quote:$uid=\\\"\\1\\\"]");
		...
		// [HS] and [/HS] for posting replies with hs, or just for quoting stuff.
	$text = bbencode_first_pass_pda($text, $uid, '[hs]', '[/hs]', '', false, '');
	$text = bbencode_first_pass_pda($text, $uid, '/\[hs=\\\\&hs;(.*?)\\\\&hs;\]/is', '[/hs]', '', false, '', "[hs:$uid=\\\"\\1\\\"]");
...
//
				// We're going to try and catch usernames with "[' characters.
				//
				if( preg_match('#\[hs=\\\&hs;#si', $possible_start, $match) && !preg_match('#\[hs=\\\&hs;(.*?)\\\&hs;\]#si', $possible_start) )
				{
					// OK we are in a quote tag that probably contains a ] bracket.
					// Grab a bit more of the string to hopefully get all of it..
					if ($close_pos = strpos($text, '&hs;]', $curr_pos + 14))
					{
						if (strpos(substr($text, $curr_pos + 14, $close_pos - ($curr_pos + 14)), '[hs') === false)
						{
							$possible_start = substr($text, $curr_pos, $close_pos - $curr_pos + 7);
						}
					}
					...
					if (sizeof($stack) > 0)
					{
						...
						if ($mark_lowest_level && ($curr_nesting_depth == 1))
						{
							if ($open_tag[0] == '[code]')
							{
								...
								$code_entities_replace = array('&lt;', '&gt;', '&quot;', '&hs;', '&#58;', '&#91;', '&#93;', '&#40;', '&#41;', '&#123;', '&#125;');

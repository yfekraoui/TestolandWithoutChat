...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		/* DEBUT : Code modifié par labyelo */
		$avatar_design = ( !empty($HTTP_POST_VARS['avatar_design']) ) ? trim(strip_tags( $HTTP_POST_VARS['avatar_design'] ) ) : '';
		/* FIN : Code modifié par labyelo */
		if( isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
		{
			...
			/* DEBUT : Code modifié par labyelo */
			$avatar_design = htmlspecialchars(stripslashes($avatar_design));
			$mood = htmlspecialchars(stripslashes($mood));
			$attributes = htmlspecialchars(stripslashes($attributes));
			$packs = htmlspecialchars(stripslashes($packs));
		    /* FIN : Code modifié par labyelo */
	...
	if( isset( $HTTP_POST_VARS['submit'] ) )
	{
		...
		if( !$error )
		{
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status WHERE user_id = $user_id";
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		/* DEBUT : Code modifié par labyelo */
		$avatar_design = htmlspecialchars($this_userdata['avatar_design']);
		$mood = htmlspecialchars($this_userdata['mood']);
		$attributes = htmlspecialchars($this_userdata['attributes']);
		$packs = htmlspecialchars($this_userdata['packs']);
		/* FIN : Code modifié par labyelo */
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
		    ...
		    /* DEBUT : Code modifié par labyelo */
			$s_hidden_fields .= '<input type="hidden" name="avatar_design" value="' . str_replace("\"", "&quot;", $avatar_design) . '" />';
			$s_hidden_fields .= '<input type="hidden" name="mood" value="' . str_replace("\"", "&quot;", $mood) . '" />';
			$s_hidden_fields .= '<input type="hidden" name="attributes" value="' . str_replace("\"", "&quot;", $attributes) . '" />';
			$s_hidden_fields .= '<input type="hidden" name="packs" value="' . str_replace("\"", "&quot;", $packs) . '" />';
			/* FIN : Code modifié par labyelo */
	...
	else
	{
		...
		$template->assign_vars(array(
			...
			/* DEBUT : Code modifié par labyelo */
			'L_AVATAR_DESIGN' => $lang['avatar_design'],
			'L_MOOD' => $lang['mood'],
			'L_ATTRIBUTES' => $lang['attributes'],
			'L_PACKS' => $lang['packs'],
			/* FIN : Code modifié par labyelo */

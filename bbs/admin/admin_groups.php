...
if ( isset($HTTP_POST_VARS['edit']) || isset($HTTP_POST_VARS['new']) )
{
	...
	$template->assign_vars(array(
		...
		/* LABYELO */
		'BLASON' => $group_info['blason'],
		'DEVISE' => $group_info['devise'], 
		'GROUP_OFFICIEL' => $group_info['group_officiel'],
		'LOCALISATION' => $group_info['LOCALISATION'], 
		/* LABYELO */
		...
		'L_YES' => $lang['Yes'],
		/* LABYELO */
        'L_BLASON' => $lang['blason'],
		'L_DEVISE' => $lang['devise'], 
		'L_GROUP_OFFICIEL' => $lang['group_officiel'],
...
else if ( isset($HTTP_POST_VARS['group_update']) )
{
	...
	else
	{
		...
		/* LABYELO */
		$blason = isset($HTTP_POST_VARS['blason']) ? trim($HTTP_POST_VARS['blason']) : '';
		$devise = isset($HTTP_POST_VARS['devise']) ? trim($HTTP_POST_VARS['devise']) : '';
		$group_officiel = isset($HTTP_POST_VARS['group_officiel']) ? trim($HTTP_POST_VARS['group_officiel']) : '';
		$LOCALISATION = isset($HTTP_POST_VARS['LOCALISATION']) ? trim($HTTP_POST_VARS['LOCALISATION']) : '';
		/* LABYELO */
		...
		if( $mode == "editgroup" )
		{
			...
			$sql = "UPDATE " . GROUPS_TABLE . "
				SET group_type = $group_type, group_name = '" . str_replace("\'", "''", $group_name) . "', blason = '" . str_replace("\'", "''", $blason) . "', devise = '" . str_replace("\'", "''", $devise) . "', group_officiel = '" . str_replace("\'", "''", $group_officiel) . "', group_description = '" . str_replace("\'", "''", $group_description) . "', group_moderator = $group_moderator, LOCALISATION = '" . str_replace("\'", "''", $LOCALISATION) . "' 
				WHERE group_id = $group_id";
			if ( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not update group', '', __LINE__, __FILE__, $sql);
		...
		{
			$sql = "INSERT INTO " . GROUPS_TABLE . " (group_type, group_name, blason, devise, group_officiel, group_description, group_moderator, group_single_user, LOCALISATION) 
				VALUES ($group_type, '" . str_replace("\'", "''", $group_name) . "', '" . str_replace("\'", "''", $blason) . "', '" . str_replace("\'", "''", $devise) . "', '" . str_replace("\'", "''", $group_officiel) . "', '" . str_replace("\'", "''", $group_description) . "', $group_moderator,	'0', '" . str_replace("\'", "''", $LOCALISATION) . "')";
			if ( !$db->sql_query($sql) )
			{

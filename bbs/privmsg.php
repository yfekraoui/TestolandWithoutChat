...
<table width="100%" cellspacing="0" border="0" cellpadding="0">
  <tr>
	...
	$temp_url = append_sid("../annuaire/?mid=" . POST_USERS_URL . '=' . $user_id_from);
	...
	if ( !$board_config['allow_html'] || !$userdata['user_allowhtml'])
	{
		if ( $user_sig != '')
		{
			$user_sig = preg_replace('#(<)([\/]?.*?)(>)#is', "&lt;\\2&gt;", utf8_decode($user_sig));
		}
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$msg_avatar = $row['avatar_design'];

		$u_from_user_profile = append_sid("../annuaire/?mid=$msg_userid");

...
if ( !$is_auth['auth_read'] || !$is_auth['auth_view'] )
{
	if ( !$userdata['session_logged_in'] )
	{
		...
		redirect(append_sid("/login/?redirect=bbs.$phpEx&$redirect", true));
	}
	//
...
if ( $mark_read == 'topics' )
{
	if ( $userdata['session_logged_in'] )
	{
		...
		$template->assign_vars(array(
			'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid("bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id") . '">')
		);
	}

	$message = $lang['Topics_marked_read'] . '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid("bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id") . '">', '</a> ');
	message_die(GENERAL_MESSAGE, $message);
}
//
...
{
	$moderators[] = '<a href="' . append_sid("/annuaire/?mid=" . $row['user_id']) . '">' . $row['username'] . '</a>';
}
...
$template->assign_vars(array(
	...
	'S_POST_DAYS_ACTION' => append_sid("bbs.$phpEx?" . POST_FORUM_URL . "=" . $forum_id . "&amp;start=$start"))
);
...
<table width="100%" cellspacing="0" border="0" cellpadding="0">
  <tr>
	...
	'U_MARK_READ' => append_sid("bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;mark=topics"))
);
//
// End header
//
...
if( $total_topics )
{
	for($i = 0; $i < $total_topics; $i++)
	{
				...
				if( $topic_rowset[$i]['post_time'] > $userdata['user_lastvisit'] ) 
				{
					if( !empty($tracking_topics) || !empty($tracking_forums) || isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_f_all']) )
					{
						...
						if( $unread_topics )
						{
							...
							$newest_post_img = '<a href="' . append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;view=newest") . '"><img src="' . $images['icon_newest_reply'] . '" alt="' . $lang['View_newest_post'] . '" title="' . $lang['View_newest_post'] . '" border="0" /></a> ';
						}
						else
						{
							$folder_image = $folder;
		...
		if( ( $replies + 1 ) > $board_config['posts_per_page'] )
		{
			...
			{
				$goto_page .= '<a href="' . append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=" . $topic_id . "&amp;start=$j") . '">' . $times . '</a>';
		...
		$view_topic_url = append_sid("topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id");

		$topic_author = ( $topic_rowset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid("/annuaire/?mid=" . $topic_rowset[$i]['user_id']) . '">' : '';
		...
		$last_post_author = ( $topic_rowset[$i]['id2'] == ANONYMOUS ) ? ( ($topic_rowset[$i]['post_username2'] != '' ) ? $topic_rowset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("/annuaire/?mid="  . $topic_rowset[$i]['id2']) . '">' . $topic_rowset[$i]['user2'] . '</a>';

		$last_post_url = '<a href="' . append_sid("topic.$phpEx?"  . POST_POST_URL . '=' . $topic_rowset[$i]['topic_last_post_id']) . '#' . $topic_rowset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
		...
		$topic_row = '<table width="100%" cellpadding="0" cellspacing="0" border="0">
<tr>
...
<td align="right" background="templates/chimboz/images/index_body_cat_sep.gif"><img src="templates/chimboz/images/index_body_cat_sep_r.gif"/></td>
	...
	$template->assign_vars(array(
		'PAGINATION' => generate_pagination("bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;topicdays=$topic_days", $topics_count, $board_config['topics_per_page'], $start),

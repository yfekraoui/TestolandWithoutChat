...
switch( $mode )
{
	case 'delete':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
  ...
  <tr>
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "bbs.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
			...
			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "bbs.$phpEx?" . POST_FORUM_URL . "=$old_forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'lock':
		...
		if ( !empty($topic_id) )
		{
			$redirect_page = "topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
			$message = sprintf($lang['Click_return_topic'], '<a href="' . $redirect_page . '">', '</a>');
		}
		else
		{
		...
		$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "bbs.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				$template->assign_vars(array(
					'META' => '<meta http-equiv="refresh" content="3;url=' . "topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">')
				);

				$message = $lang['Topic_split'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . "topic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
			}
		}
		else

		...
		if($vpid > 0)
		{
			...
			if($type == "pack") // PACK EXEMPLE BIG BASTA SANS ITEM
			{
				...
				else if($req4['money'] >= $req3['prix'])
				{
					...
					$action = 'Achat : '.$req3['name'];
					$KFC = 'INSERT INTO banque(id, user_id, action, prix, times) VALUES ("", "'.$userdata['user_id'].'", "'.$action.'", "-'.$req3['prix'].'", "'.time().'")';
			...
			else if($type == "pack_spec") // PACK EXEMPLE PINGOUIN
			{
				$obtened_head_1 = 0;
				$obtened_body_2 = 0;
				$obtened_shoes_3 = 0;
				$req4 = mysql_query('SELECT money, item0, body, shoes FROM phpbb_users WHERE user_id = "'.$userdata['user_id'].'"');
				$req4 = mysql_fetch_assoc($req4);
				$_pack =preg_split("/,/", $req3['items']);
			...
			else if($type == "pack_item0") // LUNETTE MASQUE
			{
				$obtened_item0 = 0;
			...
			else if($type == "pack_item1") // SAC AUTRE
			{
				$obtened_item1 = 0;
